...
def _assert_orm_matches_migration(connection) -> None:
    ...
    for table_name in PHASE2_TABLES:
        ...
        assert set(reflected_columns) == set(orm_table.columns)
...
def test_phase2_migration_round_trip_and_postgres_invariants(monkeypatch):
    ...
    try:
        ...
        monkeypatch.setenv("PGOPTIONS", f"-csearch_path={schema},public")
        ...
        with test_engine.connect() as connection:
            assert PHASE2_TABLES.isdisjoint(inspect(connection).get_table_names())
        ...
        with test_engine.connect() as connection:
            ...
            _assert_orm_matches_migration(connection)
        ...
        if migrated_to_phase2:
            command.downgrade(Config("backend/alembic.ini"), "0009_snapshot_lifecycle")
